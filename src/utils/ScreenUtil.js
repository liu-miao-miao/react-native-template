...
export function logConsole(val) {
    console.log(val)
}
...
export function Accuracy(num, len) {
    ...
    if (wordCount.length > 4) {
        countNum = Number(wordCount.substring(0, wordCount.length - 4) + '.' + wordCount.substring(wordCount.length - 4, wordCount.length)).toFixed(len || 0)
    }
    return countNum;
}
